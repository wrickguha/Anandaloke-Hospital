...
$age= $con->real_escape_string(trim($_POST['age']));
$gender= $con->real_escape_string(trim($_POST['gender']));
$department= $con->real_escape_string(trim($_POST['department']));
$appoint_date=$con->real_escape_string(trim($_POST['date']));
$enq_date=date('Y-m-d');
$status=0;
...
`name`='".$name."',`email`='".$email."',`phone`='".$phone."',`age`='".$age."',`gender`='".$gender."',`department`='".$department_name."',`appoint_date`='".$appoint_date."',`status`='".$status."',`enquiry_date`='". $enq_date."'";
	...
	echo "<script>alert('Mysqli Error...');location.replace('/');</script>";
}
